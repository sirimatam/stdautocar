...
while($type = pg_fetch_row($type_array))
{
	...
	while($prod = pg_query($prod_array))
	{
		...
		while($sku = pg_fetch_row($sku_array))
		{
			...
			<TD>$sku_color</TD>
			<TD>$sku_qtt</TD>
			<TD><A HREF='show_product.php?id=$sku_id'>แก้ไข</A></TD>
			</TR>";
		}
	}
}
...
<FORM METHOD=POST ACTION="edit_product_submit.php" enctype="multipart/form-data">
...
<TR>
	<TD>ไซส์</TD>
	<TD><INPUT TYPE="text" NAME="sku_size" value="<?php echo $sku_size;?>" ></TD>
</TR>
<TR>
	<TD>จำนวนสินค้า</TD>
