...
<?php
if ($_POST['track_submit'] != '')
{
	$tracking = $_POST['track_no'];
	$order_id = $_POST['track_submit'];
	pg_query($db,"UPDATE orderlist SET tracking_number = '$tracking' WHERE order_id = '$order_id'");
	pg_query($db,"UPDATE orderlist SET order_status = 'shipping' WHERE order_id = '$order_id'");
	echo "<p>หมายเลขใบสั่งซื้อ ".$order_id." จัดส่งเรียบร้อย</p>";
}
?>
